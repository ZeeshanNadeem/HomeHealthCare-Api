...
const { UserRequest } = require("../models/UserRequestSchema");
  ...
  for (let i = 0; i < staff.length; i++) {
    for (let j = 0; j < staff[i].locations.length; j++) {
      ...
      if (parseInt(radiusFound) <= parseInt(staff[i].locations[j].radius)) {
        if (!staffInRadius.includes(staff[i])) staffInRadius.push(staff[i]);
      }
    }
  }
    ...
    for (let leaveIterate of leaves) {
      ...
      if (Between || same1 || same2) {
        for (let leaveSlots of leaveIterate.slots) {
          slotBlocked = staffMember.availableTime.map((s) => {
...
const dutiesCheck = async (
  ...
  for (let s of requests) {
    ...
    if (
      date_.isBetween(leave_start_date, leave_end_date) ||
      date_.isSame(startDate) ||
...
router.get("/", paginatedResults(Staff), async (req, res) => {
  ...
  if (req.query.findStaffOnOrg) {
    ...
    const staffAfterBlockedSlotsOnLeave = await filterSlotOnLeave(
      staffBetweenRadius,
      req.query.date
    );
    const filterPendingStaff_ = await filterPendingStaff(
      staffAfterBlockedSlotsOnLeave
    );
...
router.post("/", async (req, res) => {
  ...
  if (req.query.SlotsBooked) {
    const slotsToReAssign = await dutiesCheck(
      req.body.staffID,
      req.body.leave_from,
      req.body.leave_to,
      req.body.slots
